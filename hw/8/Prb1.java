...
public class Prb1{
  public static void main(String[] args) {
    ...
    //gets user input
    System.out.print("Enter a string, press enter, then enter delimiters: ");
  ...
  public static ArrayList<String> split(String s, String delimiters){
    ...
    for (int i = 0; i < s.length(); i++){
      ...
      for (int j = 0; j < delimiters.length(); j++){
        if (s.charAt(i) == delimiters.charAt(j)){ //if delimiter is found in string
          splitString.add(s.substring(nextStartPoint,i));
          nextStartPoint = i + 1; //index where to start for next substring
        }
      }
    }
    //makes remainder of string into an item in list
    splitString.add(s.substring(nextStartPoint, s.length()));
